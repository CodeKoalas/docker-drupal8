...
if [[ -n "$CONF_REPO" && ! -z $CONF_REPO ]] ; then
  ...
  GIT_SSL_NO_VERIFY=true git clone -b $CONF_REPO_BRANCH $CONF_REPO /root/configs

  grep -q -F '/root/configs' /root/crons.conf || echo "*/15 * * * * . /root/project_env.sh; cd /root/configs && GIT_SSL_NO_VERIFY=true git pull origin $CONF_REPO_BRANCH" >> /root/crons.conf
